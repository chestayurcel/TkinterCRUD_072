...
# Fungsi untuk menangani tombol submit (Add)
def submit():
...
# Element GUI
Label(root, text="Nama Siswa", bg="#FFFFFF").grid(row=0, column=0, padx=10, pady=5)
...
Button(root, text="Add", command=submit, bg="#98FB98").grid(row=4, column=0, padx=10, pady=5)
Button(root, text="Update", command=update, bg="#87CEEB").grid(row=4, column=1, padx=10, pady=5)
Button(root, text="Delete", command=delete, bg="#FA8072").grid(row=4, column=2, padx=10, pady=5)
...
# Memuat data dari database ke dalam tabel di GUI
populate_table()
...
# Menghubungkan klik pada tabel dengan fungsi untuk mengisi input form
tree.bind("<ButtonRelease-1>", fill_inputs_from_table)

# Memulai loop utama aplikasi GUI
root.mainloop()